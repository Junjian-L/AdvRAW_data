...
raw_dir = os.path.join(dataset_dir, 'train', 'huawei_raw')
raw_dir = os.path.join(dataset_dir, 'train', 'JJ')
for i in range(TRAIN_SIZE):
    rawimg = np.asarray(imageio.imread(os.path.join(raw_dir, str(i) + '.png'))).astype(np.uint16)
    rawimg = rawimg.reshape([448, 448])
    ...
    rgbimg_ccm = rgbimg_ccm.astype(np.uint8)
    imageio.imwrite("results/generator_" + str(i) + "_epoch_" + ".jpg", rgbimg_ccm)
